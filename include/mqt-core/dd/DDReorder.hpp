...
namespace dd {
    ...
    class ReorderStepManager {
        public:
            ...
            /**
             * @brief 如果当前的内存块已经被全部使用完(即空闲链表为空,内存块也为空)时,向操作系统
             * 再次申请另一块内存块放到内存池中
             */
            void reallocChunk();
    ...
    class VarOrder {
        public:
            explicit VarOrder(MatrixDD mdd, qc::QuantumComputation *qc) :
                ...
                mdd(mdd), 
                qtc(qc), manager(new ReorderStepManager())
            {}
             ...
             */
            void record(Qubit level, ReorderScheme scheme, size_t ddSize,
                        bool up) {
             ...
             * @brief 查看记录是否为空
             */
            inline bool isRecordEmpty()
            {
                return reorderSteps.empty();
            }
        ...
        private:
            ...
            std::vector<ReorderStep*> reorderSteps; // 记录每一步进行的哪种变换,以及对应的交换层和交换方式
